...
var log = require('debug')('pm2:interface:daemon');
var fs = require('fs');
 ...
 * @param {Function} cb invoked with <result>
 */
...
InteractorDaemonizer.killInteractorDaemon = function (conf, cb) {
  ...
  InteractorDaemonizer.ping(conf, function (online) {
    log('Interactor is %d', online ? 'online' : 'offline');
...
var daemonize = function (conf, infos, cb) {
  ...
  var testEnv = process.env.TRAVIS || process.env.NODE_ENV.match(/test/);
...
InteractorDaemonizer.getOrSetConf = function (cst, infos, cb) {
  ...
  try {
    confFS = json5.parse(fs.readFileSync(cst.INTERACTION_CONF));
  ...
  try {
    fs.writeFileSync(cst.INTERACTION_CONF, json5.stringify(configuration, null, 4));
  } catch (e) {
